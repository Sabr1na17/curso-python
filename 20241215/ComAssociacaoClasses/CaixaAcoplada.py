#coding: utf-8
import time
import ControleAcionamento as CA
import ValvulaEntrada as VA
import ValvulaSaida as VS
class CaixaAcoplada(object): 
	def __init__(self):
		...
		Controle = CA.ControleAcionamento()
		ValvulaEntrada = VA.ValvulaEntradaAgua()
		ValvulaSaida = VS.ValvulaSaidaAgua()
	...
	def AcionarDescarga(self):
		...
		self.Controle.AcionarAlavanca(tipo)
		if tipo == '2':
			print("/t/t/tFedeu?")
			Resposta = input("/t/t/tDigite 1 se sim e 2 se não")
			if Resposta == '1':
				self.AcionamentoPerfume+=1
				self.Controle.set_acionamento_perfume()
			#Verificacao da troca do perfume
			Troca = self.Controle.get_informacao_troca_perfume()
			if (Troca):
				print("Precisa trocar refil do perfume")
			else:
				print("Ainda há perfume no refil")
			self.ValvulaEntrada.set_troca_refil_perfume(Troca)
