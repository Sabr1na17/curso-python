...
class PaginaLogin(tk.Frame): 
	...
	def VerificaLogin(self): 			
		...
		if Nome_usuario ==Dados[0][0]: 
			...
			tk.messagebox.showinfo("Erro", "Usuário não validado")
		if flagNome and Senha_usuario ==Dados[0][1]: 
			print("Senha Validada")
			flagSenha = True
		else: 
			print("Senha não validada")	
			tk.messagebox.showinfo("Erro", "Senha não validada")
		if flagNome and flagSenha: 
			Mensagem = "Bem-vindo "+Nome_usuario
			tk.messagebox.showinfo("Validacao", Mensagem)
			
			#self.PagUsuario = PaginaUsuario(self)
			
			self.menuUsuario = TrocadorTelaUsuario(self)	
			self.menuUsuario.grid(row = 0, column = 0, sticky = "nswe")
			
class TrocadorTelaUsuario(tk.Frame): 
	def __init__(self, JanPr): 
		tk.Frame.__init__(self,JanPr)	
		self.PagUsuario = PaginaUsuario(self)
		self.PagUsuario.grid(row=0, column = 0, sticky = "nswe")
		
		self.LabelCentral=tk.Label(self, text = "Tela Usuario", height = 5, width = 52, background = "Red")
		self.LabelCentral.grid(row = 0, column = 2, columnspan=1)
			
class PaginaUsuario(tk.Frame): 
	def __init__(self, JanPr): 
		tk.Frame.__init__(self,JanPr)
		self.Label_Usuario = tk.Label(self, text = "Pagina do usuario", height = 5, width = 40, bg = 'Blue', fg = "White")
